...
class DatatableWidget2 extends StatefulWidget {
  const DatatableWidget2({Key? key}) : super(key: key);

  @override
  State<DatatableWidget2> createState() => _DatatableWidget2State();
}

class _DatatableWidget2State extends State<DatatableWidget2> {
                ...
                return DataTable(
                  ...
                  rows: condidateViewModel
                      .historiqueElements(snapshot.data!.appointments,
                          1) // Loops through dataColumnText, each iteration assigning the value to element
                      .map(
                        ((element) => DataRow(
                              cells: <DataCell>[
                                DataCell(Text(element.sender.toString(),
                                    style: TextStyle(
                                        fontSize:
                                            15))), //Extracting from Map element the value
                                DataCell(Text(
                                    element.startedAt!.substring(0, 10),
                                    style: TextStyle(fontSize: 15))),
                                DataCell(Text(element.startedTime!,
                                    style: TextStyle(fontSize: 15))),
